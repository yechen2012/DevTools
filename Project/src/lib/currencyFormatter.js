...
OSREC.CurrencyFormatter = {
    ...
    inBuiltSymbols: ["AMD", "ARS", "AUD", "BGN", "BRL", "CAD", "CHF", "CLP", "CNY", "CZK",
        "DKK", "EUR", "GBP", "GEL", "HKD", "HRK", "HUF", "IDR", "ILS", "ISK",
        "INR", "JPY", "KRW", "KZT", "MXN", "MYR", "NOK", "NZD", "PEN", "PLN",
        "RON", "RUB", "SEK", "SGD", "THB", "TRY", "TWD", "USD", "UAH", "VND", "ZAR"],
    // inBuiltSymbols: ["ARS", "AUD", "BRL", "CAD", "CHF", "CLP", "CNY", "CZK",
    //     "DKK", "EUR", "GBP", "GEL", "HKD", "HRK", "HUF", "IDR", "ISK",
    //     "JPY", "MXN", "MYR", "NOK", "NZD", "PEN", "PLN",
    //     "RON", "RUB","SEK", "SGD", "TWD", "USD", "ZAR"],
    isInBuiltSymbols: true,
    getFormatDetails: function getFormatDetails(p) {
        ...
        var isInbuit = OSREC.CurrencyFormatter.isInBuiltSymbols;
        ...
        var localty = p.locale || defaultLocales[currency];
        if (typeof localty === 'undefined') {
            localty = 'en';
        }
        locale = locales[localty];
        ...
        if (p.symbol === undefined) {
            symbol = isInbuit ? symbols[currency] : symbols['EUR'];
        } else if (p.symbol === null) {
            symbol = '';
        } else {
            symbol = p.symbol;
        }
    ...
    getFormatter: function getFormatter(p) {
        ...
        var format = function format(n, f) {
            ...
            var replaceresult = formattedNumber.replace(/\!/g, symbol);
            //ygg版本partnerconnect.js好像有重载string的trim()方法，货币符号会被删除，ygg版本临时处理
            // var result=replaceresult.trim();
    ...
    getFormatterWithNoSym: function getFormatterWithNoSym(p) {
        var formatDetails = OSREC.CurrencyFormatter.getFormatDetails(p);
        ...
        var format = function format(n, f) {
            ...
            var replaceresult = formattedNumber.replace(/\!/g, '');
    ...
    checkIsInBuiltSy: function (sy) {
        var inbuitsy = OSREC.CurrencyFormatter.inBuiltSymbols;
        if (inbuitsy.indexOf(sy) == -1) {
            OSREC.CurrencyFormatter.isInBuiltSymbols = false;
            return false;
        }
        OSREC.CurrencyFormatter.isInBuiltSymbols = true;
        return true
    },
    //默认两位小数，无千位分割，小数分割.
    //number 数值，precision小数点位数，thousand千位分隔符，decimal，小数点符号
    formatNumber: function formatNumber(number, tprecision, tthousand, tdecimal) {
        var precision = 2;
        if (tprecision != undefined) {
            precision = tprecision;
        }
        var thousand = tthousand || "";
        var decimal = tdecimal || "."
        var tnumber = OSREC.CurrencyFormatter.toFixed(number, precision);
        var negative = number < 0 ? "-" : "";
        var base = parseInt(number) + "";
        var mod = base.length > 3 ? base.length % 3 : 0;
        var baseresult = negative + (mod ? base.substr(0, mod) + thousand : "") + base.substr(mod).replace(/(\d{3})(?=\d)/g, "$1" + thousand);
        var dec = precision > 0 ? decimal + tnumber.split('.')[1] : "";
        return baseresult + dec;
    },
    formatCustomMoney: function formatCustomMoney(n, p, precision) {
        var symbols = OSREC.CurrencyFormatter.symbols;
        var isInbuit = OSREC.CurrencyFormatter.isInBuiltSymbols;
        ...
        var number = this.formatNumber(n, precision, ',');
        return symbol + number;
    },
    formatWithNoSym: function formatWithNoSym(n, p) {
        var formatterFunction = OSREC.CurrencyFormatter.getFormatterWithNoSym(p);
        return formatterFunction(n);
    },
};
